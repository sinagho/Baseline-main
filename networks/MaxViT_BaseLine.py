...
## CBAM and SE ---> Convolutional Attn Blocks ---> just For developing default baselines
## By: Sina 
...
class BaselineAttnBlock (nn.Module):

        """

        The attention baseline

        Token Mixer (attention) can be chosen by USER
        By Default the SE (squeeze and excitation) attention is utilized.

        arguments:
            dim: Embedding dimension
            mlp_ratio: The increase rate for mlp layers (for more details check ViT)
            drop = dropout rate
            drop_path = drop_path rate (something like Drop Connect)
            act_layer = activation function
            linear: related to the configs of MLP (convmlp or linearmlp)
            reduction = Reduction Rate of SE Block FCs --> channel = 64 --> reduction = 8 ---> 64/8
        ...
        def __init__(self,
                     ...
                     linear=False):
            super().__init__()
            self.norm1 = nn.LayerNorm(dim)  # LayerNorm_1

            self.attn = SELayer(channel=dim, reduction=reduction)  # Token Mixer
            self.drop_path = DropPath(
                drop_path) if drop_path > 0. else nn.Identity()  # Drop_path

            self.norm2 = nn.LayerNorm(dim)  # build_norm_layer(norm_cfg, dim)[1] # LayerNorm_2
            mlp_hidden_dim = int(dim * mlp_ratio)  # For example: 4 * 1024
            self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                           act_layer=act_layer, drop=drop, linear=linear)  # MLP
            layer_scale_init_value = 1e-2
            self.layer_scale_1 = nn.Parameter(layer_scale_init_value * torch.ones((dim)),
                                              requires_grad=True)  # Layer_scale
...
class MyDecoderLayer(nn.Module):
    def __init__(
        ...
        if not is_last:
            ...
            # self.lka_attn = LKABlock(dim=dims)
        ...
        self.layer_Tr_1 = BaselineAttnBlock(dim= out_dim,
                                             mlp_ratio=4.,
                                             drop=0.,
                                             drop_path=0.,
                                             act_layer=nn.GELU,
...
##########################################
#
# MaxViT with Base Transformer (SE) as Decoder stuff
# By: Sina
##########################################
class MaxViT_BaseFormerDecoder(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        super().__init__()

        # Encoder
        self.backbone = MaxViT4Out_Small(n_class=num_classes, img_size=224) # Pretrained MaxViT
        ...
        reduction_ratio = [16, 8, 6, 2] # Designed for SE Blocks (based on it's paper)
...
if __name__ == "__main__":
    ...
    net = MaxViT_BaseFormerDecoder().cuda(0)
